...
from utils import *
import prompt
from data_loader import load_spatial_data_csv, get_data_info
import subprocess
import json
...
import pickle
# %%
# representetive_gene_list = ["Aqp4", "Gad1", "Gad2", "Sst", "Rbp4", "Cux2", "Synpr", "Nos1", "Cpne5", "Bdnf", "Adcyap1", "Syt6", "Pcp4", "Sla", "Ctgf", "Foxp2"]  # seurat cluster
...
# %%
config = load_config("model_config/config_zeroshot_starmap.yaml")
...
cmd = f"nohup python -u submit_end2end.py model_config/config_zeroshot_starmap.yaml {config.data_name} {config.replicate} > outs/{config.data_name}_zeroshot{config.replicate}.out 2>&1 &"
